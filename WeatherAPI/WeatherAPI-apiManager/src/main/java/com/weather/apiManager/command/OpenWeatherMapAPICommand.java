...
public class OpenWeatherMapAPICommand implements WeatherAPICommand {
    ...
    private String getJSON() {    	    	             
        ...
        try {        	
            ...
            for(int i =0; (i < listArray.length() && daysBetween(next_date, curr_date) == 0 ); i++)                         
            {
                ...
                if(daysBetween(next_date,curr_date) != 0) {
                    json.append("}");
                } else {
                    json.append("},");
                }
            ...
            curr_date = new Date();
            next_date = curr_date; 
            boolean[] daily_check = {false, false, false, false, false, false};
            int count = 0;
            int old_time_diff = 0;
            // get the daily data            
            for(int i =0; i < listArray.length(); i++)
            {
            	...
            	if(daysBetween(next_date,curr_date) != 0){
                	count ++;                	
                	old_time_diff = daysBetween(next_date,curr_date);
                	curr_date = next_date;
                }
